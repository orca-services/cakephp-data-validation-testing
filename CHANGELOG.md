...
### Changed
- **BREAKING CHANGE:** Replace `testData` prefix in all test trait method names with `assert`. E.g. `testDataValidationNotEmpty()` becomes `assertValidationNotEmpty()`.
...
### Fixed
- Ignore data validation in `testDataValidationIsUnique()` to correctly assert build rules.
